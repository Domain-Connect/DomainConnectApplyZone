...
From the specification. Records in a template have a type. Based on type
additional properties exist on the record.

Most of these map to the zone values, with some naming differences
(exceptions are txtConflict*)
...
SRV name, target, protocol, service, priority (int), weight (int), port (int),
ttl (int) SPFM host, spfRules

Variables are allowed in all of the strings (exceptions are the txtConflict*)

@ has special mean when used in two fields.

For the host/name field the values are relative. @ is special for empty or
...
For the pointsTo/target the fields are absolute. @ is special for the root
passed in but would be expanded.

So a value @ with domain=foo.com&host=bar would result in bar.foo.com in the
zone.
...
When a domain/host is allowed in the data field, this should be a fully
qualified domain name without a trailing dot.

All records havea 'type'. Depending on the type, additional fields are
required. Unless otherwise stated all data is a string.
...
SRV: name, data, ttl(int), protocol, service, priority (int), weight (int),
     port (int)
...
def process_spfm(template_record, zone_records, new_records):
    ...
    if not found_spf:
        new_records.append({'type': 'TXT',
                            'name': template_record['host'],
                            'data': ('v=spf1 ' + template_record['spfRules'] +
                                     ' -all'),
                            'ttl': 6000})
...
def process_srv(template_record, zone_records, new_records):
    ...
    for zone_record in zone_records:
        if (zone_record['type'].upper() == 'SRV' and
                zone_record['name'].lower() == template_record['name'].lower()):
            zone_record['_delete'] = 1
...
def process_ns(template_record, zone_records, new_records):
    ...
    for zone_record in zone_records:
        ...
        if (zone_record_name == template_record_name or
                zone_record_name.endswith('.' + template_record_name)):
            zone_record['_delete'] = 1
...
def process_records(template_records, zone_records, domain, host, params,
    ...
    for template_record in template_records:
        ...
        # If we passed in a group, only apply templates as part of the group
        if (group_ids and
        ...
        # Setting any record with a non root host should delete any NS records
        # at the same host.
        ...
        if template_record_type != 'NS' and template_record_name != '@':
            ...
            for zone_record in zone_records:
                if zone_record['type'].upper() == 'NS':
                    zone_record_name = zone_record['name'].lower()
                    if (template_record_name == zone_record_name or
                            template_record_name.endswith('.' +
                                                          zone_record_name)):
                        zone_record['_delete'] = 1
    ...
    # Add the records in the zone that weren't being deleted, and clean up the
    # _delete setting
    for zone_record in zone_records:
        if '_delete' in zone_record:
...
class DomainConnect(object):
    ...
    def prompt(self):
        """ /!\ deprecated! Will prompt for values for a template """
