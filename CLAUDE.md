...
- after changing `apply_template`, `process_records`, or `resolve_variables`, run the PR regression-check tool (`tools/pr_regression_check/`, see its README) before considering the change done. Treat a new failure like an unexpectedly failing test: stop and ask the user.